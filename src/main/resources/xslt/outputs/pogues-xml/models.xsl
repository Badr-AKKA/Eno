...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="driver-Global//QuestionSimple" mode="model">
		...
		<Child id="{$id}" questionType="SIMPLE" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:type="QuestionType">
			...
			<TargetMode>no-source-found!</TargetMode>
			<TargetMode>no-source-found!</TargetMode>
			<xsl:message> Variable id: <xsl:value-of select="enopogues:get-variable-id($source-context)"/></xsl:message>
			<xsl:apply-templates select="eno:child-fields($source-context)" mode="source">
	...
	<xsl:template match="driver-Global//TextDomain | driver-Global//DateTimeDomain | driver-Global//NumericDomain" mode="model">
		<xsl:param name="source-context" as="item()" tunnel="yes"/>
